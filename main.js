(function (factory) {
    ...
    ***************************************************************************** */
    ...
    var decomposition = function (cubes, scrollingUp) {
        ...
        var percentageOffset = getPercentageOffset();
        for (var i = 0; i < cubes.length; i++) {
            var cube = cubes[i];
            var rollAnim = rollAnimation(cube, scrollingUp);
    ...
    var cubesChanging = function (cubes) {
        ...
        for (var i = 0; i < cubes.length; i++) {
            ...
            for (var ii = 0; ii < sections.length; ii++) {
                ...
                var text = texts[ii][i] || texts[0].join(' ');
                // registerSectionObserver(section, cubes, text)
                var collide = areColliding(section, cube);
                if (collide) {
                    var front = cube.getElementsByClassName('front')[0];
    ...
    /**
     * Thanks to https://gist.github.com/jtsternberg/c272d7de5b967cec2d3d
     ...
     * @returns {boolean}
     */
    var areColliding = function (el1, el2) {
    ...
    var cubeBaseSideTranslate = deepFreeze({
        front: {
            translateZ: 55,
            rotateX: 0
        },
    ...
    var rollAnimation = function (cube, scrollingUp) {
        var animation = anime({
            targets: cube,
    ...
    // const shuffleBox = (cube:HTMLElement) => {
    //     // for (let i = 0 ; i < cubes.length ; i ++) {
    //     //     const cube = cubes[i]
    //         const sides = cube.getElementsByClassName('side')
    //         const shufflePosCopy = getShuffledRotationsObject({...cubeBaseSideTranslate})
    //         for (let ii = 0 ; ii < sides.length ; ii ++) {
    //             const side = sides[ii]
    //             const position = side.classList[1]
    //             const getCustom = () => {
    //                 if(position === 'front') {
    //                     return {
    ...
    var shuffleBox = function (cube) {
        var shuffleRotations = function (o) {
            var oc = JSON.parse(JSON.stringify(o));
            var keys = Object.keys(oc);
            keys.forEach(function (key) {
                var val = oc[key];
                var valKeys = Object.keys(val);
                valKeys.forEach(function (valKey) {
                    var increasor = Math.random() * 1800 - 150;
                    oc[key][valKey] = increasor;
                });
                // if(valKeys.length < 2) {
                //     oc[key]['rotateZ'] =    Math.random() * 1800 - 150
                // }
            });
            return oc;
        };
        ...
        //     const cube = cubes[i]
        var sides = cube.getElementsByClassName('side');
        var shufflePosCopy = shuffleRotations(__assign({}, cubeBaseSideTranslate$1));
        var _loop_2 = function (ii) {
            var side = sides[ii];
            var position = side.classList[1];
            ...
            anime(__assign(__assign(__assign({ targets: side }, shufflePosCopy[position]), getCustom()), { 
                // backgroundColor: '#FFF',
    ...
    var createCanvas$1 = function () {
        ...
        // for (let y = 0; y < canvasWidth / (blobSize/blobMargin); y+=blobSize + blobMargin) {
        //     for (let x = 0; x < canvasHeight / (blobSize/blobMargin); x+=blobSize + blobMargin) {
        //         allBlobs.push(new MyBlob(ctx, x+blobSize + blobMargin, y+blobSize + blobMargin, blobSize, 'rgba(255, 255, 255, 0.4)', getRandomColorMinusBlack()))
        //     }
        // }
        draw();
    ...
    var highlightText = function () {
            ...
            if (isInViewPort(target)) {
                ...
                window.setTimeout(function () {
                    if (!target.classList[1]) {
                        var backgroundColor = colorsHex[Math.floor(Math.random() * colorsHex.length)];
                        var backgroundStyle = 'linear-gradient(to right, ' +
                            backgroundColor + ',' +
        ...
        for (var i = 0; i < highlitables.length; i++) {
            highlight(highlitables[i]);
        }
    };
